...
static struct mode_config *nwl_dsi_mode_probe(struct nwl_dsi *dsi)
{
	...
	phy_mipi_dphy_get_default_config(clock,
		...
		lanes, &dphy_opts, dsi->dsi_mode_flags,
	...
	dphy_opts.lp_clk_rate = clk_get_rate(dsi->tx_esc_clk);

#if 0
	ret = generic_phy_validate(&dsi->phy, PHY_MODE_MIPI_DPHY, 0, &dphy_opts);
	if (ret) {
	...
	dsi->bitclk = bit_clk = dphy_opts.hs_clk_rate;
...
{
	struct phy_configure_opts_mipi_dphy new_cfg;
	unsigned long phy_ref_rate;
	struct mode_config *config;
	size_t i;
	const char *id;
	 ...
	 * derived from it atm.
	 */
	if (new_cfg.hs_clk_rate == dsi->dphy_cfg.hs_clk_rate)
		return;

	debug("PHY at ref rate: %lu (actual: %lu)\n",
		phy_ref_rate, clk_get_rate(dsi->phy_ref_clk));
	/* Save the new desired phy config */
	memcpy(&dsi->dphy_cfg, &new_cfg, sizeof(new_cfg));
