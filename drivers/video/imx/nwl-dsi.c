...
static u32 ui2bc(struct nwl_dsi *dsi, unsigned long long ui)
{
	...
	return DIV_ROUND_UP(ui * dsi->lanes, dsi->timings.pixelclock.typ * bpp);
}
...
{
	u32 cycles;
	struct phy_configure_opts_mipi_dphy *cfg = &dsi->dphy_cfg;
...
{
	struct phy_configure_opts_mipi_dphy *dphy_cfg = &dsi->dphy_cfg;
	int ret;
	...
	ret = generic_phy_configure(&dsi->phy, dphy_cfg);
	if (ret < 0) {
